...
// Beauty & Health category ID from your API
const CATEGORY_ID = '687e7f6d3f410756d06e04ec'
...
// Map of category IDs to their static URLs
...
export default function BeautyAndHealthContent() {
  ...
  useEffect(() => {
    async function fetchCategories() {
      try {
        ...
        // Fetch subcategories using the children endpoint
        const response = await fetch(`/api/categories/${CATEGORY_ID}/children`)
        if (!response.ok) {
      ...
      <main className="mx-auto max-w-[1500px] px-4 py-6">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Знайдіть найкращі товари для догляду за собою
          </p>
        </div>
            ...
            {isLoading ? (
              ...
              categories.map((category) => (
                <CategoryCard
                  ...
                  count={0}
                  href={SUBCATEGORY_URLS[category.id] || `/krasa-ta-zdorovya/${category.id}`}
                  iconType="sparkles"
                />
              ))
            )}
