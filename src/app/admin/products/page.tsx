      ...
      try {
        ...
        const walk = (nodes: any[]) => {
          for (const n of nodes) {
            if (!n?.id) continue
            const nameUk = n?.name?.uk || n?.name || n.id
    ...
    <div className="p-6 rounded-lg bg-white shadow border">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2">
          <a href="/admin" className="rounded-lg border px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100">↩ До адмін</a>
          <a href="/" className="rounded-lg border px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100">← На головну</a>
        </div>
      </div>
